// Ozon Cashback Calculator — полная версия
(function() {
    // Флаг, чтобы избежать повторной инициализации на одной странице
    if (window.__ozonCashbackRunning) return;
    window.__ozonCashbackRunning = true;
    ...
    // Наблюдатель за появлением таблицы при переключении вкладок
    ...
    // Основная логика: если есть iframe — открыть в новом окне, иначе запустить калькулятор
